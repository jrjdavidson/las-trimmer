    ...
    pub fn process_lidar_files(&self) -> Result<(), MyError> {
        ...
        let _ = thread::scope(|s| {
            ...
            while let Ok((index, points_vec)) = rx.recv() {
                ...
                {
                    let mut points_w = points_written.lock().map_err(|_| MyError::LockError).unwrap();
                    *points_w += no_of_points;
                }
            }

            let points_w = points_written.lock().map_err(|_| MyError::LockError).unwrap();
